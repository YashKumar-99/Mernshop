...
const sellerconvertion = async (req, res) => {
    ...
    if (sellerId) {
        try {
            ...
            const conversationRes = await Convertion.find({ members: sellerId }).select('groupTitle members lastMessage').lean();
...
const conversationwihtsellers = async (req, res) => {
    ...
    if (userId) {
        try {
            ...
            const conversationRes = await Convertion.find({ members: userId }).select('groupTitle members lastMessage').lean();
