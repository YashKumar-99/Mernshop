...
const Chat = () => {
    ...
    useEffect(() => {
        ...
        socketRef.current.on('message', (msg) => {
            ...
            // dispatch(incrementCount({ groupTitle, count }));
            ...
            if(!hasValue){
                socketRef.current.emit('removecount', { senderId, groupTitle ,reciverId});
            }
        });
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if (message.trim()) {
            console.log(newCount, "countvalue")

            socketRef.current.emit('message', { roomId: roomId, msg: message, senderId: senderId, count: newCount });

            // socketRef.current.emit('removecount', { senderId, groupTitle ,reciverId});
            setMessage('');
            setNewCount('')
        }
        let groupTitle = roomId
