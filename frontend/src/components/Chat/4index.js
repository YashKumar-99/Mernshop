...
const Chat = () => {
    ...
    useEffect(() => {
        ...
        async function getMessagesfrombackend() {
            ...
            try {
                ...
                dispatch(fetchMessagesSuccess(AllMessages?.data?.getMessages)); // Dispatch action to indicate successful message retrieval
                ...
                dispatch(fetchMessagesFailure()); // Dispatch action to indicate failure in fetching messages
    ...
    useEffect(() => {
        ...
        socketRef.current.on('message', (msg) => {
            ...
            // dispatch(setMessageArr(prevMessages => [...prevMessages, msg]));
        ...
        socketRef.current.on('messageList', ({ messages }) => {
            ...
            dispatch(setMessageArr(messages))

            console.log(message, "messageList...is here")
            // dispatch(setMessageArr([...messageArr, messages]));
            scrollToBottom();
        });
            ...
            <Box id="chatBox" className='displays' ref={chatRef} >
                {loading ? (
                    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '200px' }}>
                        <CircularProgress color="primary" size={60} thickness={4} /> {/* Customize the CircularProgress appearance */}
                    </div>
                ) : (
